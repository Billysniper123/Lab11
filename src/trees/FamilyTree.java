...
public class FamilyTree
{
    ...
    private static class TreeNode
    {
        ...
        TreeNode getNodeWithName(String targetName)
        {
            ...
            for (TreeNode child: children)
            {
                if (child.getNodeWithName(targetName) != null) {
                	return child;// If child.getNodeWithName(targetName) returns a non-null node,
        ...
        ArrayList<TreeNode> collectAncestorsToList()
        {
            ...
            TreeNode balls = this;
            while (balls.parent != null) {
            	ancestors.add(balls.parent);
            	balls = balls.parent;
            }
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException ("There is no colon in the string"); //throw a TreeException with a useful message
		...
		String childrenString = line.substring(colonIndex + 1, line.length() - 1); //The substring of line that starts just after colonIndex and goes through the end of
		...
		else
		{
			...
			if (parentNode == null){
				...
				throw new TreeException("The parent isn't found");
			}
		}
		...
		for (String s: childrenArray) {
			...
			TreeNode balls = new TreeNode (s);
			parentNode.addChild(balls);
		}
	}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null) {
			 throw new TreeException ("Your first node doesn't exist");//Throw a TreeException with a useful message
		}
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null) {
			throw new TreeException ("Your second node doesn't exist"); //Throw TreeException with a useful message
		}
